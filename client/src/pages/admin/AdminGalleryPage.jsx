...
const Toggle = ({ checked, onChange }) => (
  <div onClick={() => onChange(!checked)} style={{ width: 44, height: 24, borderRadius: 100, background: checked ? '#C9A84C' : 'var(--admin-bg-card)', position: 'relative', cursor: 'pointer', transition: 'background 0.25s', flexShrink: 0 }}>
...
const DeleteDialog = ({ img, onCancel, onConfirm, loading }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
      ...
      <Trash2 size={32} color="var(--admin-danger)" style={{ margin: '0 auto 12px' }} />
      <p style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--admin-text-primary)', fontFamily: "'Inter', sans-serif", marginBottom: 8 }}>DELETE IMAGE</p>
...
const EditModal = ({ img, onClose, onSave }) => {
  ...
  const inputSt = { background: 'var(--admin-bg-card)', border: '1px solid var(--admin-border)', color: 'var(--admin-text-primary)', padding: '10px 14px', fontFamily: "'Inter', sans-serif", fontSize: '0.85rem', width: '100%', boxSizing: 'border-box', outline: 'none', borderRadius: 0 };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
        onClick={e => e.stopPropagation()}
        style={{ background: 'var(--admin-bg-card)', border: '1px solid var(--admin-border)', maxWidth: 480, width: '100%', overflow: 'hidden', borderRadius: 0 }}>
        <img src={img.imageUrl} alt="" style={{ width: '100%', height: 200, objectFit: 'cover', display: 'block', borderRadius: 0 }} />
        <div style={{ padding: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
            <span style={{ fontSize: '0.65rem', fontFamily: "'DM Mono', monospace", letterSpacing: '0.2em', color: 'var(--admin-text-secondary)' }}>EDIT IMAGE</span>
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--admin-text-secondary)' }}><X size={16} /></button>
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={labelSt}>Caption</label>
            <input value={caption} onChange={e => setCaption(e.target.value)} style={inputSt} placeholder="Image caption…" onFocus={e => e.target.style.borderColor = '#C9A84C'} onBlur={e => e.target.style.borderColor = 'var(--admin-border)'} />
          </div>
          <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            ...
            <span style={{ fontSize: '0.83rem', color: 'var(--admin-text-primary)', fontFamily: "'Inter', sans-serif" }}>Featured Image</span>
          </div>
          <div style={{ display: 'flex', gap: 12 }}>
            <button onClick={onClose} style={{ flex: 1, padding: '12px', background: 'var(--admin-bg-card)', color: 'var(--admin-text-secondary)', border: '1px solid var(--admin-border-light)', fontFamily: "'Inter', sans-serif", fontSize: '0.82rem', cursor: 'pointer', borderRadius: 0 }}>CANCEL</button>
            <button onClick={save} disabled={saving} style={{ flex: 1, padding: '12px', background: '#C9A84C', color: 'var(--admin-text-primary)', border: 'none', fontFamily: "'Inter', sans-serif", fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', opacity: saving ? 0.7 : 1, borderRadius: 0 }}>
...
const UploadModal = ({ onClose, onUploaded }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(15,23,42,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      onClick={onClose}>
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
        onClick={e => e.stopPropagation()}
        style={{ background: 'var(--admin-bg-card)', border: '1px solid var(--admin-border)', maxWidth: 680, width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: 40, borderRadius: 0 }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 32 }}>
          <span style={{ fontSize: '0.65rem', fontFamily: "'DM Mono', monospace", letterSpacing: '0.2em', color: 'var(--admin-text-secondary)' }}>UPLOAD GALLERY IMAGES</span>
        ...
        <div onDrop={onDrop} onDragOver={onDragOver} onDragLeave={onDragLeave} onClick={() => fileRef.current?.click()}
          style={{
            border: `2px dashed ${isDragging ? '#C9A84C' : 'var(--admin-border-light)'}`, background: isDragging ? 'rgba(201,168,76,0.08)' : 'var(--admin-bg-elevated)',
          ...
          <Upload size={40} color={isDragging ? '#C9A84C' : 'var(--admin-text-tertiary)'} style={{ margin: '0 auto 12px' }} />
          <p style={{ color: 'var(--admin-text-secondary)', fontFamily: "'Inter', sans-serif", fontWeight: 300, fontSize: '1rem', margin: '0 0 6px' }}>Drag and drop images here</p>
          <p style={{ color: 'var(--admin-text-tertiary)', fontFamily: "'Inter', sans-serif", fontWeight: 300, fontSize: '0.85rem', margin: '0 0 8px' }}>or click to browse</p>
          <p style={{ color: 'var(--admin-text-tertiary)', fontFamily: "'DM Mono', monospace", fontSize: '0.6rem', letterSpacing: '0.1em' }}>Supports JPG, PNG, WEBP</p>
        </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
            {previews.map((p, i) => (
              <div key={i} style={{ position: 'relative', textAlign: 'center' }}>
                ...
                <p style={{ margin: '4px 0 0', fontSize: '0.6rem', color: 'var(--admin-text-secondary)', fontFamily: "'DM Mono', monospace", maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</p>
                <button onClick={() => removeFile(i)} style={{ position: 'absolute', top: -6, right: -6, background: 'var(--admin-danger)', border: 'none', borderRadius: '50%', width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff' }}>
                  <X size={10} />
                </button>
              </div>
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
          <div>
            ...
            <input value={caption} onChange={e => setCaption(e.target.value)} style={inputSt} placeholder="e.g. Premium Oak Veneer Closeup" onFocus={e => e.target.style.borderColor = '#C9A84C'} onBlur={e => e.target.style.borderColor = 'var(--admin-border)'} />
          </div>
          <div>
            <label style={labelSt}>Category</label>
            <select value={category} onChange={e => setCategory(e.target.value)} style={inputSt}>
        ...
        {uploading && previews.map((p, i) => (
          <div key={i} style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ fontSize: '0.65rem', color: 'var(--admin-text-secondary)', fontFamily: "'DM Mono', monospace" }}>{p.name}</span>
              <span style={{ fontSize: '0.65rem', color: '#C9A84C', fontFamily: "'DM Mono', monospace" }}>{progresses[i] || 0}%</span>
            </div>
            <div style={{ height: 3, background: 'var(--admin-bg-card)' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '12px 24px', background: 'var(--admin-bg-card)', color: 'var(--admin-text-secondary)', border: '1px solid var(--admin-border-light)', fontFamily: "'Inter', sans-serif", fontSize: '0.82rem', cursor: 'pointer', borderRadius: 0 }}>CANCEL</button>
          <button onClick={handleUpload} disabled={!files.length || uploading}
            style={{ padding: '12px 24px', background: '#C9A84C', color: 'var(--admin-text-primary)', border: 'none', fontFamily: "'Inter', sans-serif", fontSize: '0.82rem', fontWeight: 600, cursor: files.length ? 'pointer' : 'not-allowed', opacity: !files.length ? 0.5 : 1, borderRadius: 0 }}>
  ...
  return (
    <AdminLayout title="Gallery">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--admin-text-primary)', fontFamily: "'Inter', sans-serif", margin: 0 }}>GALLERY MANAGEMENT</h1>
          <p style={{ fontSize: '0.82rem', fontWeight: 300, color: 'var(--admin-text-secondary)', fontFamily: "'DM Mono', monospace", marginTop: 4 }}>{images.length} images uploaded</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => fetch()} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: 'transparent', border: '1px solid var(--admin-border-light)', color: 'var(--admin-text-secondary)', fontFamily: "'DM Mono', monospace", fontSize: '0.65rem', letterSpacing: '0.12em', cursor: 'pointer', borderRadius: 0 }}
          ...
          <button onClick={() => setShowUpload(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 24px', background: '#C9A84C', color: 'var(--admin-text-primary)', border: 'none', fontFamily: "'Inter', sans-serif", fontSize: '0.8rem', fontWeight: 600, letterSpacing: '0.1em', cursor: 'pointer', transition: 'background 0.2s', borderRadius: 0 }}
      ...
      {/* Category Tabs */}
      <div style={{ display: 'flex', gap: 0, overflowX: 'auto', marginBottom: 24, borderBottom: '1px solid var(--admin-border)' }}>
        {CATEGORIES.map(c => (
          <button key={c.value} onClick={() => setActiveCategory(c.value)}
            style={{
              ...
              color: activeCategory === c.value ? '#C9A84C' : 'var(--admin-text-secondary)',
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => <div key={i} style={{ height: 220, background: 'var(--admin-bg-elevated)', animation: 'pulse 1.5s infinite', borderRadius: 0 }} />)}
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 80, background: 'var(--admin-bg-elevated)', border: '1px solid var(--admin-border)', borderRadius: 0 }}>
          <ImageIcon size={48} color="var(--admin-text-tertiary)" style={{ margin: '0 auto 16px' }} />
          <p style={{ color: 'var(--admin-text-secondary)', fontFamily: "'Inter', sans-serif", fontWeight: 300, marginBottom: 8 }}>No images found</p>
          <p style={{ color: 'var(--admin-text-tertiary)', fontFamily: "'Inter', sans-serif", fontWeight: 300, fontSize: '0.82rem', marginBottom: 16 }}>
          ...
          <AnimatePresence>
            {filtered.map(img => (
              <motion.div key={img._id} layout exit={{ scale: 0.8, opacity: 0 }} transition={{ duration: 0.25 }}
                style={{ background: 'var(--admin-bg-elevated)', border: '1px solid var(--admin-border)', overflow: 'hidden', position: 'relative', cursor: 'default', borderRadius: 0 }}
                ...
                <div style={{ height: 220, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div className="gallery-overlay" style={{
                    ...
                    <button onClick={() => setEditImg(img)}
                      style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(15,23,42,0.8)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--admin-text-primary)' }}>
                      <Edit3 size={20} />
                    </button>
                    <button onClick={() => setDeleteImg(img)}
                      style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(15,23,42,0.8)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--admin-danger)' }}>
                      <Trash2 size={20} />
                    </button>
                  </div>
                ...
                <div style={{ padding: '14px 16px' }}>
                  <p style={{ margin: 0, fontSize: '0.82rem', color: 'var(--admin-text-primary)', fontFamily: "'Inter', sans-serif", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {img.caption || 'No caption'}
                  </p>
                  <span style={{ marginTop: 6, display: 'inline-block', background: 'var(--admin-bg-card)', color: 'var(--admin-text-secondary)', fontSize: '0.58rem', fontFamily: "'DM Mono', monospace", letterSpacing: '0.12em', textTransform: 'uppercase', padding: '3px 8px', borderRadius: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
                    <button onClick={() => toggleFeatured(img)} disabled={togglingId === img._id}
                      ...
                      {togglingId === img._id ? (
                        ...
                        <Star size={16} color={img.isFeatured ? '#C9A84C' : 'var(--admin-text-tertiary)'} fill={img.isFeatured ? '#C9A84C' : 'none'} />
                      )}
                    </button>
                    <span style={{ fontSize: '0.6rem', color: 'var(--admin-text-tertiary)', fontFamily: "'DM Mono', monospace" }}>
